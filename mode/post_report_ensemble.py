    ...
    for dataset in tasks:
        ...
        for key,item in soft_avg.items():
            if len(item) > 0:
                ...
                sharpen_wgt = 16.0 if n_class < 10 else opts.smooth_weight[0]
                weight_soft_unit[key] = np.exp((0.5+sharpen_add-item) * sharpen_wgt * np.log10(max(10,n_class-opts.too_many_class))).sum()
