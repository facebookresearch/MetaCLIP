...
def get_tokenizer(
    ...
    if tokenizer is None or tokenizer == 'None':
        ...
        def tokenize_fn(txt_list):
            ...
            if input_ids.size(0) == 1: # fix xlm tokenizer bugs on multiple eos tokens replace w unk token.
                if tokenizer.eos_token_id == 2 and (input_ids[0] == tokenizer.eos_token_id).sum() > 1:
                    eos_index = (input_ids[0] == tokenizer.eos_token_id).nonzero()[:, 0]
                    input_ids[0][eos_index[:-1]] = tokenizer.unk_token_id
